...
import modules.cookbook.ddl.ShopTogetherDDL;
import modules.cookbook.ddl.ShopTogetherJoinerDDL;
import util.API;
...
public class ShopOrderService {
	...
	public static final int ORDER_LUCKY_PAYED = 10;//已支付抽奖订单
	...
	public static boolean createOrder(boolean together,String togetherId,
		...
		order.setPlatformGetsRate(applyInfo.getFeeRate());
	...
	public static void paySuccess(ShopOrderDDL order){
		...
		if(!StringUtils.isEmpty(order.getLitterAppParams())){
			CookBookUsersDDL buyer = UserService.get(order.getBuyerUserId());
	...
	public static void doneTogether(ShopTogetherDDL together){
		...
		if(joners!=null && joners.size()>0){
			for(ShopTogetherJoinerDDL joner:joners){
				...
				CookBookUsersDDL user = UserService.get(joner.getUserId());
